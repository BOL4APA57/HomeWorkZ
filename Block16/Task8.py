...
# Что нужно сделать
# Юлий Цезарь использовал свой способ шифрования текста. Каждая буква заменялась на следующую по алфавиту
# через K позиций по кругу. Если взять русский алфавит и K, равное 3, то в слове, которое мы хотим зашифровать,
...
def crypt(s, step):
    if s in upp:
        point = upp.index(s) + step
        if point >= 33:
            point -= 33
        res = upp[point]
    elif s in low:
        point = low.index(s) + step
        if point >= 33:
            point -= 33
        res = low[point]
    else:
